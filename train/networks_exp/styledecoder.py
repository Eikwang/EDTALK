...
class Synthesis_with_ADAIN(nn.Module):
    def __init__(self, size, style_dim, motion_dim, blur_kernel=[1, 3, 3, 1], channel_multiplier=1):
        super(Synthesis_with_ADAIN, self).__init__()
        ...
        for i in range(3, self.log_size + 1):
            ...
            self.to_flows.append(ToFlow(out_channel, style_dim))
            self.fineadainresblock.append(FineADAINResBlock2d(out_channel,512))

            in_channel = out_channel

        self.n_latent = self.log_size * 2 - 2

    def forward(self, wa, alpha, feats, exp_feature=None):
        ...
        i = 1
        for conv1, conv2, to_rgb, to_flow, feat, adain in zip(self.convs[::2], self.convs[1::2], self.to_rgbs,
                                                       self.to_flows, feats, self.fineadainresblock):
            out = conv1(out, latent[:, i])
            out = conv2(out, latent[:, i + 1])
            if out.size(2) == 8:
                feat = adain(feat, exp_feature)
                out_warp, out, skip_flow = to_flow(out, latent[:, i + 2], feat) # torch.Size([1, 512, 8, 8])
                skip = to_rgb(out_warp)
            else:
                feat = adain(feat, exp_feature)
...
class Synthesis_with_ADAIN2(nn.Module):
    def __init__(self, size, style_dim, motion_dim, blur_kernel=[1, 3, 3, 1], channel_multiplier=1):
        super(Synthesis_with_ADAIN2, self).__init__()
    ...
    def forward(self, wa, alpha, feats, exp_feature=None):
        ...
        for conv1, conv2, to_rgb, to_flow, feat, adain in zip(self.convs[::2], self.convs[1::2], self.to_rgbs,
            ...
            if out.size(2) == 8:
                ...
                out_warp, out, skip_flow = to_flow(out, latent[:, i + 2], feat) # torch.Size([1, 512, 8, 8])
                out_warp = adain(out_warp, exp_feature)
                skip = to_rgb(out_warp)
            else:
                out_warp, out, skip_flow = to_flow(out, latent[:, i + 2], feat, skip_flow) # torch.Size([1, 512, 16, 16])
                out_warp = adain(out_warp, exp_feature)
...
class Synthesis_with_warp(nn.Module):
    def __init__(self, size, style_dim, motion_dim, blur_kernel=[1, 3, 3, 1], channel_multiplier=1):
        super(Synthesis_with_warp, self).__init__()
        ...
        for i in range(3, self.log_size + 1):
            ...
            self.to_flows.append(ToFlow(out_channel, style_dim))
            self.to_flows2.append(ToFlow2(out_channel, style_dim))
    ...
    def forward(self, wa, alpha, feats, exp_feature=None):
        ...
        i = 1
        for conv1, conv2, to_rgb, to_flow, feat, to_flow2 in zip(self.convs[::2], self.convs[1::2], self.to_rgbs,
                                                       self.to_flows, feats, self.to_flows2):
            out = conv1(out, latent[:, i])
            out = conv2(out, latent[:, i + 1])
            if out.size(2) == 8:
                out_warp, out, skip_flow = to_flow(out, latent[:, i + 2], feat) # torch.Size([1, 512, 8, 8])
                out_warp = to_flow2(out_warp, exp_feature, out_warp)
                skip = to_rgb(out_warp)
            else:
                out_warp, out, skip_flow = to_flow(out, latent[:, i + 2], feat, skip_flow) # torch.Size([1, 512, 16, 16])
                out_warp = to_flow2(out_warp, exp_feature, out_warp)
...
class Synthesis_with_warp_ADAIN(nn.Module):
    def __init__(self, size, style_dim, motion_dim, blur_kernel=[1, 3, 3, 1], channel_multiplier=1):
        super(Synthesis_with_warp_ADAIN, self).__init__()
        ...
        for i in range(3, self.log_size + 1):
            ...
            self.to_flows.append(ToFlow(out_channel, style_dim))
            self.adain.append(ADAIN(out_channel,512))
            self.to_flows2.append(ToFlow2(out_channel, style_dim))
    ...
    def forward(self, wa, alpha, feats, exp_feature=None):
        ...
        i = 1
        for conv1, conv2, to_rgb, to_flow, feat, to_flow2, adain in zip(self.convs[::2], self.convs[1::2], self.to_rgbs,
                                                       self.to_flows, feats, self.to_flows2, self.adain):
            out = conv1(out, latent[:, i])
            out = conv2(out, latent[:, i + 1])
            if out.size(2) == 8:
                feat = adain(feat, exp_feature) # torch.Size([10, 512, 8, 8])
                out_warp, out, skip_flow = to_flow(out, latent[:, i + 2], feat) # torch.Size([1, 512, 8, 8])
                out_warp = to_flow2(out_warp, exp_feature, out_warp)
                skip = to_rgb(out_warp)
            else:
...
class Synthesis_with_EAM(nn.Module):
    def __init__(self, size, style_dim, motion_dim, blur_kernel=[1, 3, 3, 1], channel_multiplier=1):
        super(Synthesis_with_EAM, self).__init__()
        ...
        for i in range(3, self.log_size + 1):
            ...
            self.to_flows.append(ToFlow(out_channel, style_dim))
            self.eams.append(EAModule(512,out_channel))
    ...
    def forward(self, wa, alpha, feats, exp_feature=None):
        ...
        i = 1
        for conv1, conv2, to_rgb, to_flow, feat, eam in zip(self.convs[::2], self.convs[1::2], self.to_rgbs,
                                                       self.to_flows, feats, self.eams):
            out = conv1(out, latent[:, i])
            out = conv2(out, latent[:, i + 1])
            if out.size(2) == 8:
                out_warp, out, skip_flow = to_flow(out, latent[:, i + 2], feat) # torch.Size([1, 512, 8, 8])
                out_warp = eam(out_warp, exp_feature)
                skip = to_rgb(out_warp)
            else:
                out_warp, out, skip_flow = to_flow(out, latent[:, i + 2], feat, skip_flow) # torch.Size([1, 512, 16, 16])
                out_warp = eam(out_warp, exp_feature)
...
class Synthesis_with_EAM2(nn.Module):
    def __init__(self, size, style_dim, motion_dim, blur_kernel=[1, 3, 3, 1], channel_multiplier=1):
        super(Synthesis_with_EAM2, self).__init__()
    ...
    def forward(self, wa, alpha, feats, exp_feature):
