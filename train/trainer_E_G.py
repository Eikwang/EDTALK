...
class Trainer(nn.Module):
    ...
    def sample(self, img_source, img_target):
        with torch.no_grad():
            ...
            # vgg_loss = self.criterion_vgg(img_recon, img_target).mean()
            # l1_loss = F.l1_loss(img_recon, img_target)
            # gan_g_loss = self.g_nonsaturating_loss(img_recon)

            # g_loss = vgg_loss + l1_loss + gan_g_loss
        # return img_recon, g_loss
