...
class Direction(nn.Module):
    def __init__(self, motion_dim):
        ...
        self.weight = nn.Parameter(torch.randn(512, motion_dim)) # torch.Size([512, 20])

    def forward(self, input):
        # input: torch.Size([1, 20])
        ...
        if input is None:
            ...
            input_diag = torch.diag_embed(input)  # torch.Size([1, 20, 20])
...
class Synthesis(nn.Module):
    ...
    def forward(self, wa, alpha, feats):
        ...
        if alpha is not None:
            ...
            if len(alpha) > 1:
                directions_target = self.direction(alpha[0])  # target torch.Size([1, 512])  self.direction是512*20的向量
                directions_source = self.direction(alpha[1])  # source
                directions_start = self.direction(alpha[2])  # start
                latent = wa + (directions_target - directions_start) + directions_source # torch.Size([1, 512])
            else:
