...
class Encoder(nn.Module):
    ...
    def forward(self, input_source, input_target, h_start=None): # torch.Size([1, 20])

        if input_target is not None:

            h_source, feats = self.net_app(input_source) # h_source: torch.Size([1, 512]) 
            h_target, _ = self.net_app(input_target) # torch.Size([1, 512])

            h_motion_target = self.fc(h_target) # torch.Size([1, 20])

            if h_start is not None:
                h_motion_source = self.fc(h_source) # h_motion_source torch.Size([1, 20]) h_start torch.Size([1, 20])
